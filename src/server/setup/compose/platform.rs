...
use crate::server::application::rerank::{RerankService, RerankerRegistry};
...
use crate::server::infrastructure::embedding::{
    cloudflare as cf_embedder, llama_server as llama_server_embedder, ollama as ollama_embedder,
};
use crate::server::infrastructure::llm::{
    llama_server_generation_client as llama_server_llm, ollama_generation_client as ollama_llm,
    workers_ai_generation_client as workers_ai_llm,
};
use crate::server::infrastructure::rerank::{
    llama_server_rerank_client as llama_server_rerank,
    workers_ai_rerank_client as workers_ai_rerank,
};
...
pub struct PlatformDeps<'a> {
    ...
    pub llama_server_api: Arc<LlamaServerApi>,
...
impl PlatformServices {
    pub fn build(deps: PlatformDeps<'_>) -> Result<Self, SetupError> {
        ...
        llama_server_embedder::register(&mut embedders, Arc::clone(&llama_server_api));
        ...
        llama_server_llm::register(&mut generation_clients, Arc::clone(&llama_server_api));
        workers_ai_llm::register(&mut generation_clients, Arc::clone(&cf_api));
        let generation_clients = Arc::new(generation_clients);
        let generation_service = GenerationService::new(
            Arc::clone(&generation_clients),
            Arc::clone(&repos.generation_model),
        );

        let mut rerankers = RerankerRegistry::new();
        llama_server_rerank::register(&mut rerankers, Arc::clone(&llama_server_api));
        workers_ai_rerank::register(&mut rerankers, Arc::clone(&cf_api));
        let rerankers = Arc::new(rerankers);
        let rerank_service =
            RerankService::new(Arc::clone(&rerankers), Arc::clone(&repos.reranker_model));
