    ...
    A: Aggregate + 'static,
{
    async fn load(&self, stream_id: Uuid) -> Result<Option<AggregateSnapshot<A>>, EsError> {
        ...
        .await
        .map_err(|e| EsError::Storage(format!("load snapshot: {e}")))?;
        ...
        let aggregate: A = serde_json::from_value(snapshot)
            .map_err(|e| EsError::Serialization(format!("deserialize snapshot: {e}")))?;
    ...
    async fn save(&self, snapshot: &AggregateSnapshot<A>) -> Result<(), EsError> {
        let snapshot_json = serde_json::to_value(&snapshot.aggregate)
            .map_err(|e| EsError::Serialization(format!("serialize snapshot: {e}")))?;
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| EsError::Storage(format!("save snapshot: {e}")))?;
